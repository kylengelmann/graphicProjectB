...
CCamera.prototype.setPerspective = function(fov, aspect, near, far) {

	this.iNear = near;
	this.iFar = far;
	var ang = Math.PI * fov/360;
	this.iTop = Math.tan(ang)*near;
	this.iBot = -this.iTop;
...
CCamera.prototype.setPix = function(xmax, ymax) {
	this.xmax = xmax;
	this.ymax = ymax;
...
var posU = this.iLeft + (xpos+.5)*this.ufrac; 	// U coord,
var posV = this.iBot  + (ypos+.5)*this.vfrac;	// V coord,
...
// stored in our CImgBuf object for the pixel at (xpix,ypix)
		...
		//
		if(name == undefined) name = ' ';
		var idx = (ypix*this.xSiz + xpix)*this.pixSiz;
		console.log('CImgBuf::' + this.constructor.name + '.iBuf:\t' + this.iBuf[idx] + ',\t' +
			 this.iBuf[idx+1] + ',\t' + this.iBuf[idx+2]);
		console.log('         ' + this.constructor.name + '.fBuf:\t' + this.fBuf[idx]
		+',\t'+  this.fBuf[idx+1] + ',\t'+  this.fBuf[idx+2]);
		
}

CImgBuf.prototype.makeRayTracedImage = function(camera) {
//==============================================================================
// TEMPORARY!!!! 
...
// Create an image by Ray-tracing.   (called when you press 'T' or 't')
 	...
 	var myCam = camera;	// the 3D camera that sets eyeRay values
 	var myGrid = new rayGrid();
 	var colr = vec4.create();	// floating-point RGBA color value
	var hit = 0;
	var skyCol = vec4.fromValues(.69, .81, 1, 1);
	
 	for(var j=0; j< this.ySiz; j++) {						// for the j-th row of pixels
	  	for(var i=0; i< this.xSiz; i++) {					// and the i-th pixel on that row,
		  	var idx = (j*this.xSiz + i)*this.pixSiz;	// Array index at pixel (i,j) 
			myCam.setEyeRay(eyeRay,i,j);						  // create ray for pixel (i,j)
			hit = myGrid.findHit(eyeRay);						// trace ray to the grid
			if(hit==0) {
...
// --lamp[0] is a point-light source at location (5,5,5).
	...
	if(width && height) {
		this.imageBuffer = new CImgBuf(width, height);
	}
	else {
	...
	this.background.color = vec4.fromValues(.69, .81, 1, 1);
	this.ambient = vec4.fromValues(1, 1, 1, 1);
	// this.ambient = this.background.color;
}

CScene.prototype.traceRay = function(ray) {
	var colr = vec4.create();
	var hitList = new CHitList();
	sky = new CHit();
	sky.material = this.background;
	hitList.addHit(sky);
	for(var n = 0; n < this.Geometry.length; n++) {
		hits = this.Geometry[n].findHit(ray);
		if(hits != -1) {
			hitList.addList(hits);
		}
	}
	return hitList;
}

CScene.prototype.traceImage = function(tick) {
	tracing = true;
	var eyeRay = new CRay();	// the ray we trace from our camera for each pixel
  	var colr = vec4.create();	// floating-point RGBA color value
	var hit = 0;
	var skyCol = vec4.fromValues(.69, .81, 1, 1);
	var myGrid = this.Geometry[0];
	var antiAliasing = this.imageBuffer.antiAliasing;
	var hitList;

	for(var i = 0; i < this.imageBuffer.fBuf.length; i ++) {
		this.imageBuffer.fBuf[i] = 0;
	}
	var j = 0;

	var scene = this;

	var drawFunc = setInterval(function(scene) {
		scene.imageBuffer.float2int();
		makeRayTex(scene.imageBuffer);
		drawAll();
	}, 1000, scene);
	...
	var traceFunc = function() {
		...
		// makeRayTex(scene.imageBuffer);
		// drawAll();
		// for(var addJ = 0; addJ < 1; addJ ++, j ++) {
		  	for(var i=0; i< scene.imageBuffer.xSiz; i++) {
		  		var idx = (j*scene.imageBuffer.xSiz + i)*scene.imageBuffer.pixSiz;
			  	if(antiAliasing > 0) {
			  		scene.imageBuffer.fBuf[idx   ] = 0;
				  	scene.imageBuffer.fBuf[idx +1] = 0;
				  	scene.imageBuffer.fBuf[idx +2] = 0;
			  		for(var m = 0; m < antiAliasing; m++) {
						for(var n = 0; n < antiAliasing; n++) {
							scene.Camera.setEyeRay(eyeRay, i + (m + Math.random())/antiAliasing, j + (n + Math.random())/antiAliasing);
							hitList = scene.traceRay(eyeRay);
							vec4.copy(colr, hitList.first.material.shade(hitList.first, scene));
						  	scene.imageBuffer.fBuf[idx   ] += colr[0]/(antiAliasing*antiAliasing);
						  	scene.imageBuffer.fBuf[idx +1] += colr[1]/(antiAliasing*antiAliasing);
						  	scene.imageBuffer.fBuf[idx +2] += colr[2]/(antiAliasing*antiAliasing);
						}
					}
				}
				else {
					scene.Camera.setEyeRay(eyeRay, i, j);
					hitList = scene.traceRay(eyeRay);
					vec4.copy(colr, hitList.first.material.shade(hitList.first, scene));
				  	scene.imageBuffer.fBuf[idx] = colr[0];
				  	scene.imageBuffer.fBuf[idx+1] = colr[1];
				  	scene.imageBuffer.fBuf[idx+2] = colr[2];
				}
		  	}
		  	if(j >= scene.imageBuffer.ySiz) {
		  		// clearInterval(traceFunc);
		  		clearInterval(drawFunc);
		  		scene.imageBuffer.float2int();
		  		tracing = false;
		  		makeRayTex(scene.imageBuffer);
		  		Keys.updateAtEnd();
		  		for(var key in Keys.keyboard) {
					Keys.keyboard[key].pressed = false;
				}
		  		tick();
		  		return;
		  	}
		// }
		j++;
		requestAnimationFrame(traceFunc, g_canvasID);
	};
	traceFunc();
}
...
CHitList.prototype.addHit = function(hit) {
	this.hits.push(hit);
	if(hit.t > 0.1) {
		if(this.first == null) {
			this.first = hit;
		}
		else if(hit.t < this.first.t) {
			this.first = hit;
		}
	}
}

CHitList.prototype.addList = function(hitList) {
	if(this.first == null) {
		this.first = hitList.first;
		this.hits = this.hits.concat(hitList.hits);
	}
	else if(hitList.first != null) {
		if(hitList.first.t < this.first.t && hitList.first.t > 0.1) {
			this.first = hitList.first;
		}
		this.hits = this.hits.concat(hitList.hits);
	}
...
CHitList.prototype.popFirst = function() {
	var ret = this.first;
	this.hits.splice(this.hits.indexOf(ret), 1);
	var t = Infinity;
	this.first = null;
	for(var i = 0; i < this.hits.length; i ++) {
		if(this.hits[i].t <= t && this.hits[i].t > 0) {
			this.first = this.hits[i];
			t = this.hits[i].t;
		}
	}
	return ret;
