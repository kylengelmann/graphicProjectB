...
/*
	...
	*/
	...
	var ray = new CRay();
	ray.orig = this.world2model * inRay.orig;
	ray.dir = this.world2model * inRay.dir;
	ray.orig = inRay.orig;
	ray.dir = inRay.dir;
	...
	var x = (ray.orig[0] + ray.dir[0]*t)%this.xgap;
	var y = (ray.orig[1] + ray.dir[1]*t)%this.ygap;
	if(x < 0) x += this.xgap;
	if(y < 0) y += this.ygap;
	if((x < this.lineWidth) || 
		(y < this.lineWidth)) {
...
// --lamp[0] is a point-light source at location (5,5,5).
	...
	this.Geometry = [];
	this.Lights = [];
	this.background = new Material([.69, .81, 1, 1]);
}
...
CHitList.prototype.addHit = function(hit) {
	...
	else if(hit.t < this.first.t && this.hit.t >= 1) {
		this.first = hit;
	}
}
