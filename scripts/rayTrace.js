...
CScene.prototype.traceImage = function() {
	var eyeRay = new CRay();	// the ray we trace from our camera for each pixel
  	var colr = vec4.create();	// floating-point RGBA color value
	var hit = 0;
	var skyCol = vec4.fromValues(.69, .81, 1, 1);
	...
	for(var j=0; j< this.imageBuffer.ySiz; j++) {
	  	for(var i=0; i< this.imageBuffer.xSiz; i++) {
	  		var idx = (j*this.imageBuffer.xSiz + i)*this.imageBuffer.pixSiz;
		  	if(antiAliasing > 0) {
		  		this.imageBuffer.fBuf[idx   ] = 0;
			  	this.imageBuffer.fBuf[idx +1] = 0;
			  	this.imageBuffer.fBuf[idx +2] = 0;
		  		for(var m = 0; m < antiAliasing; m++) {
					for(var n = 0; n < antiAliasing; n++) {
						this.Camera.setEyeRay(eyeRay, i + (m + Math.random())/antiAliasing, j + (n + Math.random())/antiAliasing);
						hitList = this.traceRay(eyeRay);
						vec4.copy(colr, hitList.first.material.shade(hitList.first, this));
					  	this.imageBuffer.fBuf[idx   ] += colr[0]/(antiAliasing*antiAliasing);
					  	this.imageBuffer.fBuf[idx +1] += colr[1]/(antiAliasing*antiAliasing);
					  	this.imageBuffer.fBuf[idx +2] += colr[2]/(antiAliasing*antiAliasing);
					}
				}
			}
			else {
				this.Camera.setEyeRay(eyeRay, i, j);
				hitList = this.traceRay(eyeRay);
				vec4.copy(colr, hitList.first.material.shade(hitList.first, this));
			  	this.imageBuffer.fBuf[idx   ] = colr[0];
			  	this.imageBuffer.fBuf[idx +1] = colr[1];
			  	this.imageBuffer.fBuf[idx +2] = colr[2];
			}
	  	}
  	}
	this.imageBuffer.float2int();
}
